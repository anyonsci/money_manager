...
import type { TransactionFormValues } from '../types/index.js';
import { ArrowRight, CheckCircle2, AlertCircle, Loader2, Sparkles, Plus } from 'lucide-react';
import { formatCurrency, formatInputDate } from '../utils/formatters.js';
import { ALLOWED_CATEGORIES, isAllowedCategory, getCanonicalCategory } from '../constants/categories.js';
...
export const HomePage: React.FC = () => {
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      await addTransaction(result.values);
      const currency = activeWorkspace?.defaultCurrency || 'USD';
      const desc = `${formatCurrency(Number(result.values.amount), currency)} (${result.values.category}${result.values.subCategory ? ` . ${result.values.subCategory}` : ''}) paid via ${result.values.account}`;
      setLastSavedSummary(desc);
      setStatus({
        type: 'success',
        message: 'Saved successfully! Posted to double-entry ledger.'
      });
      setInput('');
    } catch (err: any) {
      console.error(err);
    ...
    <div className="min-h-[70vh] flex flex-col items-center justify-center px-4 py-8 max-w-2xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="w-full space-y-4">
        <div className="relative group">
          ...
          <button
            type="submit"
            disabled={saving || isSubmitting || !input.trim() || (parsed !== null && !parsed.valid)}
          ...
          >
            {saving || isSubmitting ? <Loader2 size={20} className="animate-spin" /> : <ArrowRight size={20} />}
          </button>
        </div>
            ...
            {parsed.valid && parsed.values ? (
              <div className="flex flex-wrap items-center gap-2 text-slate-300">
                <span className="font-semibold text-emerald-400">
                  {formatCurrency(Number(parsed.values.amount), currency)} ({parsed.values.type})
                </span>
