import { DoubleSpaceQuickEntryParser } from '../src/domain/doubleSpaceQuickEntryParser';

describe('DoubleSpaceQuickEntryParser', () => {
  const parser = new DoubleSpaceQuickEntryParser();

  describe('canParse', () => {
    it('returns true when input contains two or more consecutive spaces', () => {
      expect(parser.canParse('30  HDFC  food')).toBe(true);
      expect(parser.canParse('30   Checking')).toBe(true);
    });

    it('returns false when input does not contain double space', () => {
  ...
  describe('parse', () => {
    it('parses complete double-space delimited input', () => {
    ...
    it('returns undefined if fewer than 3 parts or cannot parse', () => {
  ...
  describe('detectField', () => {
    it('detects active field by cursor position', () => {
      ...
      // At category
      const f3 = parser.detectField(input, 13);
